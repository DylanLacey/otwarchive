class UsersController < ApplicationController
  ...
  def check_account_creation_status
    ...
    if !@admin_settings.account_creation_enabled?
      setflash; flash[:error] = ts("Account creation is suspended at the moment. Please check back with us later.")
      redirect_to root_path and return
    elsif @admin_settings.creation_requires_invite?
      token = params[:invitation_token]
      if token.blank?
        if !@admin_settings.invite_from_queue_enabled?
          setflash; flash[:error] = ts("Account creation currently requires an invitation. We are unable to give out additional invitations at present, but existing invitations can still be used to create an account.")
          redirect_to root_path and return
        else
          setflash; flash[:error] = ts("To create an account, you'll need an invitation. One option is to add your name to the automatic queue below.")
