    ...
    if !@admin_settings.account_creation_enabled?
      ...
      if token.blank?
        if !@admin_settings.invite_from_queue_enabled?
          setflash; flash[:error] = ts("Account creation currently requires an invitation. We are unable to give out further invitations at present, but existing invitations can still be used to create an account.")
          redirect_to root_path and return
        else
